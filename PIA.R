...
for(i in 1 : length(etfs)){
  if(i == 1){  
    datos <- getSymbols(etfs[i], src ='yahoo', 
    from = startdate, to = enddate, auto.assign = FALSE)[,4]
    }
  else{
    datos <- cbind(datos, getSymbols(etfs[i], src ='yahoo', 
    from = startdate, to = enddate, auto.assign = FALSE)[,4])
  }  
}
colnames(datos) <- etfs
nombres <- etfs
plot(datos, main ="Historico de 5 años de precios de los ETF")
...
cholesky <- t(chol(varianza))              # 1.2: Descomposicion de Cholesky(L) 

# Etapa 2: Proyecciones
years <- 1                                 # 2.1: Definir los años a simular
simulaciones <- 100                        # 2.1: Definir el numero de escenarios  
d <- datos[nrow(datos),]                   # 2.1: Precios del ultimo dia de la serie (d)
...
forecasts <- list() # En la variable forecasts se guardaran las proyecciones     
for(k in 1 : simulaciones){
  forecast <- matrix(as.numeric(d), nrow = 1, ncol = ncol(datos2))
  for(i in 1 : (252*years)){
    randomnumbers <- as.matrix(c(rnorm(ncol(datos2), 0, 0.5)), 
    ncol = 1, nrow = ncol(datos2))                                  # 2.2: Generacion de vector de numeros aleatorios (x)
    v <- t(cholesky %*% randomnumbers) + medias                     # 2.3: u + Lx
    forecast <- rbind(forecast, forecast[nrow(forecast),]*(v + 1))  # 2.4: Transfrmacion de retornos a precios
  }
  forecast <- forecast[-1,]
...
for(v in 1 : ncol(datos)){
  ...
  for(i in 2:simulaciones){
    instrumento1 <- merge(instrumento1, rbind(datos[,v], 
    forecasts[[i]][,v]))
  }
...
for(i in 1 : length(forecasts)){
  ...
  ac <- as.xts(ac, order.by =  index(forecasts[[i]])) * 
              capitalinicial
...
plot(PortafoliosEW, main = paste0("Portafolios EW, Media = ", 
round(mean(PortafoliosEW[nrow(PortafoliosEW),]), 2)))

# Mean Variance Optimization

library(quadprog) # Para solver cuadratico
library(ggplot2)
...
for(i in ps){
  retornosanualizados <- rbind(retornosanualizados, 
  (as.numeric(datos[i]) / as.numeric(datos[i-252])) - 1)
}
...
for(u in 1 : length(Rt)){
  ...
  A <- cbind(suma_weights_A, 
       retorno_minimo_esperado_A, wnn0A, wnn1A)
  
  # Optimizar
  qp <- solve.QP(varianza_de_retornos, 
        vector_de_retornos_esperados, A, b0, meq = 2)
  
  if(u == 1){
    ws <- qp$solution
    ws[which(ws<0)] <- 0
    varport <- sqrt(t(qp$solution) %*% varianza_de_retornos 
    %*% qp$solution)
  }
  else{
    ws <- rbind(ws, qp$solution)
    ws[which(ws<0)] <- 0
    varport <- rbind(varport, sqrt(t(qp$solution) %*% 
    varianza_de_retornos %*% qp$solution))
  }
}
...
meanvarianceeficiente <- meanvariance[c(which.min(
  meanvariance$Riesgo) : nrow(meanvariance)),]
ggplot(meanvarianceeficiente[,c(1,2)], aes(x = Riesgo, y = Retorno)) +
  geom_point(aes(color = Riesgo), size = 1)

# Seleccionar retorno y nivel de resgo deseado

w <- as.numeric(meanvarianceeficiente[1, 
c(3 : ncol(meanvarianceeficiente))])
names(w) <- etfs
...
plot(PortafoliosMV, main = paste0("Portafolios MV, Media = ", 
    round(mean(PortafoliosMV[nrow(PortafoliosMV),]),2)), 
    col = palette("Dark 2"))
...
plot(PortafoliosHRP, main = paste0( "Portafolios HRP, Media = ", 
    round(mean(PortafoliosHRP[nrow(PortafoliosHRP),]),2)), 
